...
public class zalogGUI extends JFrame implements ActionListener{
	JPanel mainPanel;
	JButton button_done;
	...
	public static void createfile(String year, int firstdays, int seconddays, String naryad, String bumaga) throws IOException {
		...
		if(Integer.parseInt(year) < 2020) {
			File directory = new File("C:\\Public\\NotariusDB\\zalog\\20142019");
			int fileCount = directory.list().length + 1;
			FileWriter writehandle = new FileWriter("C:\\Public\\NotariusDB\\zalog\\20142019\\"+ fileCount +".txt");
			BufferedWriter writer = new BufferedWriter(writehandle);
			writer.write(firstdays + "_" + seconddays + "_" + bumaga + "_" + naryad);
			writer.close();
			writehandle.close();
		} else {
			File directory = new File("C:\\Public\\NotariusDB\\zalog\\20202029");
			int fileCount = directory.list().length + 1;
			FileWriter writehandle = new FileWriter("C:\\Public\\NotariusDB\\zalog\\20202029\\"+ fileCount +".txt");
			BufferedWriter writer = new BufferedWriter(writehandle);
			writer.write(firstdays + "_" + seconddays + "_" + bumaga + "_" + naryad);
			writer.close();
			writehandle.close();
